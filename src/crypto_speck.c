...
static crypto_speck_Ret crypto_speck_speck64_expand_key(
    ...
    // Copy original key into working buffer
    u32 tmp[3];
    tmp[0] = speck64_key.ptr[0];
    tmp[1] = speck64_key.ptr[1];
    tmp[2] = speck64_key.ptr[2];

    // The first round key is the last word of the original key
    round_keys.ptr[0] = speck64_key.ptr[3];

    // Iteratively generate the remaining 26 round keys
    for (u32 i = 0U; i < (CRYPTO_SPECK_SPECK64_ROUNDS - 1U); i++) {
        u32 idx = i % 3U;

        // Advance key state using ARX structure
        tmp[idx] = (CRYPTO_SPECK_ROTR32(tmp[idx], 8U) + round_keys.ptr[i]) ^ i;
        u32 round_key_tmp = round_keys.ptr[i];
        round_keys.ptr[i + 1U] = CRYPTO_SPECK_ROTL32(round_key_tmp, 3U) ^ tmp[idx];
    }
